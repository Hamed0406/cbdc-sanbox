...
| Method | Path | Description |
|--------|------|-------------|
| `POST` | `/merchant/register` | Register a merchant profile; returns one-time API key. |
| `GET` | `/merchant/profile` | Get the authenticated merchant's profile. |
| `POST` | `/merchant/payment-requests` | Create QR payment request with expiry. |
| `GET` | `/merchant/payment-requests` | List payment requests (paginated, filterable by status). |
| `GET` | `/merchant/payment-requests/{id}` | Get a specific payment request. |
| `DELETE` | `/merchant/payment-requests/{id}` | Cancel a PENDING payment request. |
| `POST` | `/payments/qr` | Pay a QR code (customer endpoint, any authenticated user). |

### QR URI Format

```
cbdc://pay?merchant={uuid}&amount={cents}&ref={ref}&expires={unix}&sig={hmac}
```

QR codes expire after 15 minutes (configurable via `expiry_seconds`). The `sig` is HMAC-SHA256 over `merchant={id}&amount={cents}&expires={unix}` using the server signing key. The server re-verifies the signature before processing payment — tampering the amount or merchant ID invalidates the signature.
...
4. Runs all 12 schema migrations
5. Seeds demo users, wallets, and balances (`make seed` / `cmd/seed`)
...
| 2 | Authentication (register, login, JWT, refresh) | ✅ Complete | ✅ Complete |
| 3 | Wallets & ledger core | ✅ Complete | ✅ Complete |
| 4 | CBDC issuance (admin) | ✅ Complete | ✅ Complete |
| 5 | P2P payments (send, history) | ✅ Complete | ✅ Complete |
| 6 | WebSocket live notifications | ✅ Complete | ✅ Complete |
| 7 | Merchant & QR payments | ✅ Complete | 🔲 Pending |
| 8 | Refunds | 🔲 Pending | 🔲 Pending |
| 9 | Admin dashboard (freeze, audit log) | 🔲 Pending | 🔲 Pending |
| 10 | PWA (service worker, offline mode) | — | 🔲 Pending |
| 11 | Seed data & demo scenarios | ✅ Complete | — |
